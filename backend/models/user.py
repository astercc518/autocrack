...
class User(db.Model):
    ...
    # created_attacks = db.relationship('Attack', backref='creator', lazy='dynamic')  # 暂时注释
...
def init_default_roles():
    ...
    if not manager_role:
        manager_role = Role(
            name='manager',
            display_name='数据管理员',
            description='负责数据管理和分配的管理员'
        )
        # 添加数据管理相关权限
        manager_permissions = [
            Permission.TARGET_CREATE, Permission.TARGET_READ, Permission.TARGET_UPDATE,
            Permission.ATTACK_CREATE, Permission.ATTACK_READ, Permission.ATTACK_UPDATE,
            Permission.PROXY_READ, Permission.PROXY_UPDATE,
            Permission.DATA_IMPORT, Permission.DATA_EXPORT, Permission.DATA_CLEAN, Permission.DATA_DISTRIBUTE
        ]
    ...
    if not operator_role:
        operator_role = Role(
            name='operator',
            display_name='操作员',
            description='执行攻击任务的操作员'
        )
        # 添加操作相关权限
        operator_permissions = [
            Permission.TARGET_READ, Permission.ATTACK_CREATE, Permission.ATTACK_READ, 
            Permission.ATTACK_EXECUTE, Permission.PROXY_READ, Permission.DATA_IMPORT
        ]
        for permission in operator_permissions:
            operator_role.add_permission(permission)
    ...
    if not viewer_role:
        viewer_role = Role(
            name='viewer',
            display_name='查看者',
            description='只能查看数据的只读用户'
        )
        # 添加只读权限
        viewer_permissions = [
            Permission.TARGET_READ, Permission.ATTACK_READ, Permission.PROXY_READ
        ]
...
def create_admin_user(username="admin", password="admin123", email="admin@autocrack.com"):
    """创建默认管理员用户"""
    ...
    if not admin_user:
        ...
        try:
            db.session.commit()
            print(f"✅ 默认管理员用户创建完成: {username}/{password}")
        except Exception as e:
            db.session.rollback()
            print(f"❌ 管理员用户创建失败: {e}")
    else:
